...
BEGIN_MESSAGE_MAP(CDirect3DTestDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_EXIT, &CDirect3DTestDlg::OnBnClickedExit)
	ON_BN_CLICKED(IDC_3DOBJECT, &CDirect3DTestDlg::OnBnClicked3dobject)
	ON_BN_CLICKED(IDC_FISH, &CDirect3DTestDlg::OnBnClickedFish)
	ON_WM_TIMER()
	ON_WM_DESTROY()
END_MESSAGE_MAP()
...
BOOL CDirect3DTestDlg::OnInitDialog()
{
	...
	if (!m_key.Create(this))
		AfxMessageBox(L"DirectInput nicht verfügbar");
	//SetTimer(1, 100, NULL);
...
void CDirect3DTestDlg::DisableButtons() {
	...
	GetDlgItem(IDC_3DOBJECT)->EnableWindow(FALSE);
}
...
void CDirect3DTestDlg::OnBnClickedExit()
{
	OnOK();
}


void CDirect3DTestDlg::OnBnClicked3dobject() {
	MSG msg;
	...
	int modell[][2] = {                 // Modell (Rechteck mit Textur) 
		{ 0, 2 }, { 1, 3 }, { 3, 0 },
		{ 1, 3 }, { 3, 0 }, { 4, 1 },     // {vertex_nr, textur_nr}
	};

	o[0].BuildTexturedVertexes(m_d3d.m_pd3dDevice, vertices, textures, modell, 2); // anz Dreiecke
	o[0].SetTextureFromFile(L"ingress.jpg");
	o[0].Move(0.0f, 0.0f, 0.0f);
	o[0].Rotate(1.5707f, 0.0f, 0.0f);

	o[1].BuildTexturedVertexes(m_d3d.m_pd3dDevice, vertices, textures, modell, 2); // anz Dreiecke
	o[1].SetTextureFromFile(L"ingress.jpg");
	...
	for (m_run = true; m_run;) {              // animation loop
		...
		o[1].Rotate(0.0f, 0.05f, 0.0f);
		m_d3d.BeginRender();
		if (!m_d3d.Render(o, 2)) { //anz Objekte
			AfxMessageBox(L"Szene konnte nicht gerendert werden"); return;
		}
		CString s;
		s.Format(L"%.2f fps\nDirect3D-Beispielanwendung", m_d3d.m_fps);
...
void CDirect3DTestDlg::OnBnClickedFish()
{
	MSG msg;
	...
	o[0].BuildFromObtFile(m_d3d.m_pd3dDevice, "m_fish1.obt", 0.0025f, 0xFF00BFFF);
	o[0].Rotate(-1.55f, 0.0f, 0.0f);
	o[0].Move(1.f, 0.0f, 0.0f);
	...
	for (m_run = true; m_run;) {              // animation loop
		...
		if (m_key.KeyPressed(DIK_UP))
			o[0].Rotate(0.0f, 0.0f, 0.2f);
		if (m_key.KeyPressed(DIK_DOWN))
			o[0].Rotate(0.0f, 0.0f, -0.2f);
		if (m_key.KeyPressed(DIK_LEFT))
			o[0].Rotate(0.0f, -0.2f, 0.0);
		if (m_key.KeyPressed(DIK_RIGHT))
			o[0].Rotate(0.0f, 0.2f, 0.0);
...
void CDirect3DTestDlg::OnTimer(UINT_PTR nIDEvent)
{
	//if (m_key.KeyPressed(DIK_UP))
	//	o[0].Rotate(0.0f, 0.0f, 0.2f);
	//if (m_key.KeyPressed(DIK_DOWN))
...
void CDirect3DTestDlg::OnDestroy()
{
	CDialog::OnDestroy();
